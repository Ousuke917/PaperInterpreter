...
2.  InterpreterMCP の PDF_to_Markdown_Converter を使用して、PDFファイルをMarkdown形式に変換し、テキストと画像を抽出する。
    ...
    - 一応、dest_dir内のmaterialsに画像が保存されているかどうかをコマンドで確認すること。
3. 1番の手順で理解した内容をもとに、学生の知識レベルに合わせた解説レポートを作成する。作成場所については、dest_dir内にreport.mdという名前で保存すること。
    ...
    - 画像の挿入や配置の仕方については適宜考えながら、必要に応じて画像サイズを知育して横に並べたりするなど工夫うして、レポートの見栄えを良くすること。
        - 例えば、Markdownで画像を横に並べるには、HTMLの`<img>`タグを使用する方法等があります。以下はその例です：
        ```markdown
        <div style="display: flex; gap: 10px;">
            <img src="path/to/image1.png" alt="Image 1" style="width: 45%;">
            <img src="path/to/image2.png" alt="Image 2" style="width: 45%;">
        </div>
        ```
        - 適宜このようなHTMLタグの使用による工夫を行って画像配置を工夫・調整してください。
